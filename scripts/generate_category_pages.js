...
const categoryPageTemplate = `import { getProductsByCategory, getProductsByCategoryAndBrand, getAllProducts } from '@/lib/products'
...
validCategories.forEach(category => {
  ...
  const categoryDir = path.join(__dirname, '..', 'app', category);
  if (!fs.existsSync(categoryDir)) {
    fs.mkdirSync(categoryDir, { recursive: true });
  }
  
  const pageContent = categoryPageTemplate
    .replace(/CATEGORY_SLUG/g, category)
    .replace(/CATEGORY_NAME/g, categoryName);
  
  fs.writeFileSync(path.join(categoryDir, 'page.tsx'), pageContent);
  console.log(`✅ Created ${category}/page.tsx`);
});

console.log(`\n✅ Generated ${validCategories.length} category pages`);
